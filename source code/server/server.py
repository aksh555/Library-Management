...
def addBorrower():
    ...
    try:
        cursor.execute(
            query, (card_id, data["ssn"], data["name"], data["address"], data["phone"]))
        ...
        if(err.errno in errorCodes.errorCodeMessage):
            ...
            response = {
                'message': 'Borrower Creation failed!', 'success': False}
...
def searchBook():
    ...
    ISBNList = []
    try:
        cursor.execute(query1, ("%"+searchQuery+"%", "%" +
        ...
        if(len(ISBNList) > 0):
            ...
            query2 = 'select b.ISBN,b.Title,GROUP_CONCAT(a.name),b.isCheckedOut from BOOK b left outer join BOOK_AUTHORS ba on b.isbn = ba.isbn left outer join AUTHORS a on ba.author_id = a.author_id where b.isbn IN ' + \
            ...
            response = {'searchResult': searchResult,
                        'message': 'search success', 'success': True}
        else:
            response = {'searchResult': None,
                        'message': 'search success', 'success': True}
    except mysql.connector.Error as err:
        if(err.errno in errorCodes.errorCodeMessage):
            response = {'searchResult': None,
            ...
            response = {'searchResult': None,
                        'message': 'search failed', 'success': False}
    return jsonify(response)
...
def checkoutBook():
    ...
    try:
        ...
        if(isBorrower):
            ...
            if(isCheckedOut):
                response = {
                    'message': 'Book already checked out!', 'success': False}
            elif(isCountExceeded):
                response = {
                    'message': 'Maximum limit of 3 reached. You cannot checkout', 'success': False}
            else:
...
def searchBookLoan():
    ...
    try:
        ...
        if(err.errno in errorCodes.errorCodeMessage):
            ...
            response = {'searchResult': None,
                        'message': 'searchFailed', 'success': False}
    return jsonify(response)
...
def checkinBook():
    ...
    try:
        ...
        if(err.errno in errorCodes.errorCodeMessage):
            ...
            response = {'message': 'Book Check In failed!', 'success': False}
    return jsonify(response)
    ...
    for row in rows:
        ...
        try:
            ...
            if(err.errno in errorCodes.errorCodeMessage):
                ...
                response = {
                    'message': 'Fine calculation failed', 'success': False}
    return jsonify(response)
...
def fetchFines():
    ...
    try:
        ...
        for row in rows:
            ...
            for row1 in cursor:
                type_fixed_row = tuple(
                    [el.decode('utf-8') if type(el) is bytearray else el for el in row1])
                childSet.append(type_fixed_row)
            childMap[cardId] = childSet
            type_fixed_row = tuple(
                [el.decode('utf-8') if type(el) is bytearray else el for el in row])
            resultSet.append(type_fixed_row)
        response = {'aggregateData': resultSet, 'finesDataForCardId': childMap,
                    'message': 'Fines updated!', 'success': True}
    except mysql.connector.Error as err:
        if(err.errno in errorCodes.errorCodeMessage):
            response = {
                'message': errorCodes.errorCodeMessage[err.errno], 'success': False}
